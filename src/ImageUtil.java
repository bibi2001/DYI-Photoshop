...
class ImageUtil {
	...
	//EXCEÇOES
	 static void validateNullPoint(ColorImage img){
			if(img==null)
				 throw new NullPointerException("Your argument can't be a null image!");
	 }
	 
	 static void validateNullPoint(Layer layer){
			if(layer==null)
				 throw new NullPointerException("Your layer can't be null!");
	 }
	 
	 
	 static void validateIllegalArgumentException(int x, int y){
			if(x<0||y<0)		
				throw new IllegalArgumentException("Your arguments can't be negative!");
	 }
	 
	 static void validateIllegalArgumentException(double x){
			if(x<0)
				throw new IllegalArgumentException("Your factor can't be negative!");
	 }
	 
	 static void validateIllegalArgumentException(int x, int y, int width, int height){
		 if(x>=width ||y>=height)
				throw new IllegalArgumentException("This layer has its image outside of your poster!"); 
	 }
	 
	 static void isFull(int nextLayer, int y){
		 if(nextLayer>=y)
			 throw new IllegalStateException("Your Poster is full! If you want more space, use doubleSizerInc! Or you can just remove a layer.");
	 }
	 
	 static void validateIllegalArgument(int x){
		if(x==0)
			throw new IllegalArgumentException("You can't change your background layer!");
	 }
